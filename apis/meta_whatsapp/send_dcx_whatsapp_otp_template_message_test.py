...
def test_meta_whatsapp_otp_adapter_builds_expected_template_payload(monkeypatch) -> None:
    ...
    monkeypatch.setenv("META_WHATSAPP_OTP_TEMPLATE_NAME", "dcx_login_otp")
    monkeypatch.setenv("META_WHATSAPP_OTP_TEMPLATE_LANGUAGE_CODE", "en")
    ...
    result = send_dcx_whatsapp_otp_template_message(
    ...
    assert sent_request["request_url"] == "https://graph.facebook.com/v21.0/123456789/messages"
    assert sent_request["request_headers"]["Authorization"] == "Bearer test_token"
    assert sent_request["request_payload"]["to"] == "+34600000001"
    assert sent_request["request_payload"]["template"]["name"] == "dcx_login_otp"
    assert sent_request["request_payload"]["template"]["components"][0]["parameters"][0]["text"] == "123456"
...
def test_meta_whatsapp_otp_adapter_raises_clear_error_when_required_config_missing(monkeypatch) -> None:
    ...
    try:
        ...
        assert str(exc) == (
            ...
            "META_WHATSAPP_TOKEN,META_PHONE_NUMBER_ID,META_WHATSAPP_OTP_TEMPLATE_NAME"
        )
...
def test_meta_whatsapp_otp_adapter_raises_clear_error_when_provider_send_fails(monkeypatch) -> None:
    ...
    try:
        ...
        assert str(exc) == "API_DCX_WHATSAPP_OTP_PROVIDER_SEND_FAILED"
    else:  # pragma: no cover - defensive
