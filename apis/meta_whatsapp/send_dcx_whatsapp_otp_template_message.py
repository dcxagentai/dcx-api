"""
CONTEXT:
This file sends one WhatsApp OTP template message through the Meta WhatsApp Cloud API.
It exists so the first DCX account-phone linking flow can deliver one verification code
without leaking provider-specific payload details into account or auth capabilities.
"""
...
def send_dcx_whatsapp_otp_template_message(
    phone_e164: str,
    otp_code: str,
    ...
    CONTRACT:
      preconditions:
        ...
        - otp_code is one non-empty one-time verification code string.
        ...
        - META_WHATSAPP_OTP_TEMPLATE_NAME is configured in the backend environment.
      postconditions:
        - Sends one WhatsApp template message carrying the OTP code to the target phone.
    ...
    NARRATIVE:
      WHY this exists:
        - The first production WhatsApp identity step should use one provider adapter instead of
          scattering Meta payload shape and environment handling across account capabilities.
      WHEN TO USE it:
        - Use it after one WhatsApp OTP challenge has been prepared and persisted successfully.
      WHEN NOT TO USE it:
      ...
      WHAT COMES NEXT:
        - Later WhatsApp flows can add richer template variants or non-template senders behind
          the same adapter folder without changing the account-phone capabilities.

    TESTS:
      - meta_whatsapp_otp_adapter_builds_expected_template_payload
      - meta_whatsapp_otp_adapter_raises_clear_error_when_required_config_missing
      - meta_whatsapp_otp_adapter_raises_clear_error_when_provider_send_fails

    ERRORS:
      - API_DCX_WHATSAPP_OTP_PROVIDER_CONFIGURATION_MISSING:
          suggested_action: Configure the required Meta WhatsApp environment values before sending OTP codes.
          common_causes:
    ...
    meta_whatsapp_otp_template_name = os.getenv("META_WHATSAPP_OTP_TEMPLATE_NAME", "").strip()
    meta_whatsapp_otp_template_language_code = (
        os.getenv("META_WHATSAPP_OTP_TEMPLATE_LANGUAGE_CODE", "").strip() or "en"
    )
    ...
    if meta_whatsapp_otp_template_name == "":
        missing_config_vars.append("META_WHATSAPP_OTP_TEMPLATE_NAME")

    if len(missing_config_vars) > 0:
        raise RuntimeError(
            "API_DCX_WHATSAPP_OTP_PROVIDER_CONFIGURATION_MISSING:" + ",".join(missing_config_vars)
        )

    if not isinstance(phone_e164, str) or phone_e164.strip() == "":
        raise RuntimeError("API_DCX_WHATSAPP_OTP_PROVIDER_SEND_FAILED")

    if not isinstance(otp_code, str) or otp_code.strip() == "":
        raise RuntimeError("API_DCX_WHATSAPP_OTP_PROVIDER_SEND_FAILED")
    ...
    request_payload = {
        ...
        "template": {
            ...
            "language": {
                "code": meta_whatsapp_otp_template_language_code,
            },
            "components": [
                {
                    "type": "body",
    ...
    try:
        ...
        raise RuntimeError("API_DCX_WHATSAPP_OTP_PROVIDER_SEND_FAILED") from exc
