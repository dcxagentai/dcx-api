"""
CONTEXT:
This file prepares one pending WhatsApp OTP challenge for linking a phone number to an
already authenticated DCX account.
It exists so the app can start phone verification without writing an unverified phone number
into the user's live confirmed profile fields.
"""
...
def prepare_authenticated_dcx_user_whatsapp_phone_link_otp_delivery(
    ...
    CONTRACT:
      ...
      postconditions:
        ...
        - Returns the raw OTP code for immediate provider delivery unless the phone is already confirmed for this user.
      side_effects:
    ...
    NARRATIVE:
      ...
      WHEN TO USE it:
        - Use it when an authenticated app user enters or resends a phone-link WhatsApp code.
      WHEN NOT TO USE it:
        - Do not use it to verify an OTP or to process inbound WhatsApp webhooks.
      WHAT CAN GO WRONG:
      ...
      WHAT COMES NEXT:
        - The caller sends the raw OTP through the provider adapter, marks the delivery as sent, and then returns the refreshed account summary.
    ...
    ERRORS:
      ...
      - API_AUTHENTICATED_DCX_USER_WHATSAPP_PHONE_LINK_SEND_LIMIT_REACHED:
          ...
          common_causes:
            - too many WhatsApp OTP sends in one short window
          recovery_steps:
    ...
    otp_code = (otp_code_provider or generate_dcx_whatsapp_phone_link_otp_code)()
    otp_salt = (otp_salt_provider or generate_dcx_whatsapp_phone_link_otp_salt)()
    otp_hash = hash_dcx_whatsapp_phone_link_otp_code(otp_code=otp_code, otp_salt=otp_salt)
    challenge_expires_at_ts_ms = now_ts_ms + DCX_WHATSAPP_PHONE_LINK_OTP_LIFETIME_MS
            ...
            with connection.cursor() as cursor:
                ...
                if active_challenge_row is None:
                    cursor.execute(
                        ...
                        VALUES (%s, NULL, %s, %s, %s, %s, %s, %s, NULL, NULL, 0, %s, 0, NULL, 1, %s, %s, NULL, NULL, %s, %s, %s)
                        RETURNING id
                        """,
                        (
                            authenticated_user_id,
