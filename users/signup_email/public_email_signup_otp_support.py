...
def build_public_email_signup_otp_email_draft(language_code: str, normalized_email: str, otp_code: str) -> dict:
    ...
    if language_code == "es":
        ...
        subject = "Votre code d’accès DCX"
        text_body = (
            "Bonjour,\n\n"
            f"Votre code d’accès DCX est : {otp_code}\n\n"
            "Il expire dans 10 minutes.\n"
            "Si vous n’avez pas demandé ce code, vous pouvez ignorer cet e-mail."
        )
    elif language_code == "de":
        subject = "Ihr DCX-Zugangscode"
        text_body = (
            "Hallo,\n\n"
            f"Ihr DCX-Zugangscode lautet: {otp_code}\n\n"
            "Er läuft in 10 Minuten ab.\n"
...
def build_public_email_signup_otp_email_delivery_draft(
    ...
    if language_code == "es":
        ...
        link_line = f"\n\nPoursuivez la vérification ici :\n{verification_link_url}"
    elif language_code == "de":
        link_line = f"\n\nSetzen Sie die Verifizierung hier fort:\n{verification_link_url}"
    else:
...
def build_public_email_signup_confirmation_email_delivery_draft(
    ...
    if language_code == "fr":
        return {
            "recipient_email": normalized_email,
            "subject": "Vous êtes sur la liste d’attente de DCX Agentic",
            "text_body": (
                "Bonjour,\n\n"
                "Merci d’avoir rejoint la liste d’attente de DCX Agentic.\n"
                "Nous avons bien votre adresse e-mail et partagerons davantage d’informations très bientôt.\n\n"
                "Merci,\n"
                "DCX"
            ),
        }
    if language_code == "de":
        return {
            "recipient_email": normalized_email,
            "subject": "Sie stehen auf der Warteliste von DCX Agentic",
            "text_body": (
                "Hallo,\n\n"
                "Vielen Dank, dass Sie sich für die Warteliste von DCX Agentic angemeldet haben.\n"
                "Wir haben Ihre E-Mail-Adresse erfasst und werden bald weitere Informationen mit Ihnen teilen.\n\n"
                "Danke,\n"
                "DCX"
            ),
        }
